...
- Check that `VITE_BACKEND_URL` was set during build
- Verify backend Cloud Run service is deployed and public

### Multiple Three.js instances warning

- Make sure all `three/examples/jsm/*` imports use `three-stdlib` instead
- Already configured in `vite.config.ts` with alias and dedupe

### CORS errors

- Backend has CORS enabled for all origins
- If issues persist, check backend logs in Cloud Run
